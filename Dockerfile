...
ENV CXX11="ON" BOOST="OFF" SHARED="ON" CHAR_SIGN="unsigned" API_VERSION="3" X3_PARSE="ON"

CMD CMAKE_CXX_COMPILER="${CXX}" CMAKE_C_COMPILER="${CC}" CMAKE_LIBRARY_PATH="${BASE}/usr/lib:${BASE}/build" GTEST_ROOT="${BASE}/usr" BOOST_ROOT="${BASE}/usr" CFLAGS="-Werror -g" CXXFLAGS="-Werror -g" ci/build_cmake.sh
